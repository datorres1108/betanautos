...
{oRequest.finishRequest=function(){return xajax.responseReceived(oRequest);}
}
...
function xajax_cargando(){

//imagen de lectura de xajax
/* */

//Muestre siempre reloj
	if ( document.getElementById( "loading-indicator" )){
		// nothing
	} else {
		var divTag = document.createElement("div");
		divTag.id = "loading-indicator";
		...
		divTag.style.background="url(funciones/xajax_core/cargando3.gif) no-repeat center center fixed";
		divTag.style.opacity=".4";
	...
	xajax.callback.global.onFailure = function(args){
		var divTag = document.createElement("div");
		divTag.id = "error-alert";
		divTag.style.position="fixed";
		divTag.style.top="50%";
		divTag.style.left="50%";
		divTag.style.width= "900px";
		divTag.style.minHeight= "200px";
		divTag.style.maxHeight= "500px";
		divTag.style.marginTop= "-200px";
		divTag.style.marginLeft= "-450px";
		...
		divTag.className ="dynamicDiv";
		divTag.innerHTML = args.request.responseText;
		divTag.innerHTML += "<center><input type='button' class='boton_gris' onclick=this.parentNode.parentNode.style.display='none' value='Cerrar'></center>";
		document.body.appendChild(divTag); 		
	}	
}

var VentanaLBN = function (id,nombre) {
    this.nombre = nombre;
    this.id=id;

    var div_posicion = "relative";
    var div_top = "60px";
    var div_margin = "60 auto 0 auto";
    var div_width = "300px";
    var div_height = "150";

   // var div_backgroundColor = "#CCCCCC";
    var div_id=id;
    var div_contenido="";
    ...
    function crear_() {
		...
		divTag.style.position=div_posicion;
		divTag.style.width= div_width;
		divTag.style.top= div_top;
		divTag.style.margin= div_margin;
		divTag.style.height= div_height;
		//divTag.style.backgroundColor=div_backgroundColor;
        //divTag.style.border="1px solid #666666";
        //divTag.style.borderColor="#999999";

		//divTag.className ="fondo_titulos dynamicDiv1";
		divTag.className ="dynamicDiv1";
        //divTag.style.paddingBottom = "1px";
  		//divTag.innerHTML="<img style=\"position: absolute; top:2px; right: 2px; cursor:pointer;\" src=\"imagenes/sistemas/del6.png\" onclick=\"document.body.removeChild(document.getElementById(\'"+divTag.id+"\'))\"><fieldset style=\"padding: 0 0 0 0; border: 0px; height:"+(div_height-2)+"\"><legend style='height: 28px; font-size: 14px; width: 99%; margin: 0 -5px -5px 0; padding: 5px 0 0 0 ; text-align: center; cursor:move;' onmousedown=\"dragStart(event,'"+div_id+"')\" class=\"titulos\" ><b>"+nombre+"</b>&nbsp;</legend><div class=\"tabla_blanca fondo_tablas\" style=\"width:100%;height:100%;overflow-y:auto;\" id=\""+divTag.id+"_contenido\">"+div_contenido+"</div></fieldset>";
        divTag.innerHTML="<table class='tabla_movible borde_redondo' cellpadding='0' cellspacing='0' border='0' width='100%' ><tr><th nowrap='nowrap' class='tabla_movible_titulo' onmousedown=\"dragStart(event,'"+div_id+"')\" ><div class='borde_redondo'>"+nombre+" <img style='cursor:pointer; margin-bottom:-8px;' src=\"imagenes/sistemas/del6.png\" onclick=\"document.body.removeChild(document.getElementById(\'"+divTag.id+"\'))\"></div></th></tr><tr><td class='tabla_movible_cuerpo'><div class='tabla_movible_cuerpo_contenedor' style=\"overflow-y:auto;height:"+(div_height-2)+"\" id=\""+divTag.id+"_contenido\">"+div_contenido+"</div></td></tr></table>";
		if (!document.getElementById(div_id))
			document.body.appendChild(divTag);
    }
    function set_(campo,valor) {
        if(campo=='posicion') div_posicion=valor;
        if(campo=='top') div_top=valor;
        if(campo=='margin') div_margin=valor;
        if(campo=='width') div_width=valor;
        if(campo=='height') div_height=valor;
        if(campo=='backgroundColor') div_backgroundColor=valor;
    }

    this.crear = function() {
        crear_();
    }
    this.set = function(campo,valor) {
        set_(campo,valor);
    }
    this.contenido = function(content) {
        div_contenido=content;
    }
}
function EliminaVentanaLBN(id) {
        document.body.removeChild(document.getElementById(id));
}

function VentanaLBN2 (options) {
	if(!options.width)options.width=300;
	if(!options.height)options.height=100;
	if(!options.content)options.content="<div id='"+options.id+"_contenido'></div>";
	if(!document.getElementById(options.id))
		$.newWindow(options);
	var left=($(window).width()/2) -(options.width/2);
	if(!options.content)var top=($(window).height()/2) -(options.height/2); else var top = options.top;
	top= top+window.pageYOffset;
	$.moveWindow(options.id, left, top);
}
function EliminaVentanaLBN2(id) {
       $.closeWindow(id);
}
