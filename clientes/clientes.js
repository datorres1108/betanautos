...
//convertir en mayuscula lo texteado
function mayus(e) {
...
function AgregarVehiculo()
{
    ...
    if(docu=="")
    {
      alerta("Debes de ingresar un documento del cliente para registrar el vehiculo","Creacion de clientes");
    }
    else
    {
      ...
      if(placa=="")
      {
        alerta("Debes de ingresar una placa de vehiculo","Creacion de clientes");
      }
      else
      {
        xajax.call('CtrGuardaVehiculo',{parameters:[placa,docu,modelo,tipo,color,motor],mode:'synchronous'});
      }
...
{
  var form = xajax.getFormValues('crear_cliente');
  xajax.call('CtrGuardaCliente', {parameters:[form],mode:'synchronous'});
}
...
function ConsultaClientes()
{
    ...
    xajax.call('CtrEditaConsultaCliente', {parameters:[opcion,valor],mode:'synchronous'});
}

function GuardarEdicionVeh(placa,id)
{
  var form = xajax.getFormValues('editar_vehiculo');
  xajax.call('CtrGuardaEdivVehiculo', {parameters:[form,placa,id],mode:'synchronous'});
}

function GuardarEdicionClie(id,opcion,valor)
{
  var form = xajax.getFormValues('editar_cliente');
  xajax.call('CtrGuardaEdiCliente', {parameters:[form,opcion,valor,id],mode:'synchronous'});
}


function EliminaVehiculo(placa,id,opcion,valor)
{
    ...
    title: titulo,
    message: "Esta Seguro(a) de eliminar la placa "+placa+" ?",
    buttons: [
            {
                ...
                action: function(dialogRef)
                {
                 ...
                 xajax.call('EliminaPlaca',{parameters:[id,placa,opcion,valor],mode:'synchronous'});  
                  dialogRef.close();
                }           
            },
            {
